...
CONTAINS
  ...
  SUBROUTINE SetupAtomMovements(IErr)
    ...
    !If required, change the atomic position basis so that the crystal symmetry won't
    !be broken in atomic refinement
    IBasisChangeFLAG=0
    ...
    DO ind = 1,SIZE(IAtomsToRefine)
       ...
       DO jnd = 1,IDegreesOfFreedom(ind)
          ...
          IF(my_rank.EQ.0) THEN
             PRINT*, "RVector(knd,:) = ", RVector(knd,:)
             PRINT*, "IAtomMoveList(knd) = ",IAtomMoveList(knd)
          END IF
       END DO
    END DO
