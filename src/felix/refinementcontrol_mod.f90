...
MODULE refinementcontrol_mod
  IMPLICIT NONE
  PRIVATE
  PUBLIC :: SimulateAndFit,SimulateWithGrid, Simulate, FigureOfMeritAndThickness
  ...
  SUBROUTINE SimulateWithGrid(IErr)
    ...
    USE IPARA, ONLY : INoOfVariables, nReflections, IAbsorbFLAG, INoofUgs, &
         IPixelCount, ISimFLAG, ISymmetryRelations, IUgOffset, IRefineMode, &
         IEquivalentUgKey, IAtomstoRefine, ISizeofGrid, IThicknessCount
    USE RPARA, ONLY : RAngstromConversion,RElectronCharge,RElectronMass,&
         RConvergenceAngle, RMinimumGMag, RTolerance, RRelativisticCorrection, &
         RVolume, RgMatrix, RgMatrixMagnitude, RCurrentGMagnitude,Rhkl,RBasisAtomPosition
    USE RConst, ONLY : RPlanckConstant
    ...
    INTEGER(IKIND),INTENT(OUT) :: IErr
    INTEGER(IKIND) :: ind,jnd,knd, IThicknessIndex, IGridLength
    CHARACTER*100 :: SFormat,SPrintString
    REAL(RKIND) :: RCurrentG(3), RScatteringFactor
...
!!$       END DO
    ...
    !2D grid - should be degrees of freedom, not number of atoms - will change in future
    !    ELSE IF(SIZE(IAtomsToRefine).EQ.2) THEN
    ...
    DO knd=1,SIZE(RGridValues(:,1))
       DO ind=1,SIZE(IAtomsToRefine)
          IF (IAtomsToRefine(ind).EQ.1) THEN
             RBasisAtomPosition(IAtomsToRefine(ind),1)=RGridValues(knd,1)
          ELSE
             RBasisAtomPosition(IAtomsToRefine(ind),3)=RGridValues(knd,2)
          END IF
       END DO
       CALL UniqueAtomPositions(IErr)
       IF(l_alert(IErr,"SimulateAndFit","UniqueAtomPositions")) RETURN
       ...
       IF(my_rank.EQ.0) THEN
          ...
          DO IThicknessIndex = 1,IThicknessCount
             CALL WriteIterationOutput(knd,IThicknessIndex,0,IErr)
