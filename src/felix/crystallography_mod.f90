...
MODULE crystallography_mod
  ...
  !!
  !! Major-Authors: Keith Evans (2014), Richard Beanland (2016, 2017), Alex Hubert (2018)
  !!
  SUBROUTINE UniqueAtomPositions(IErr)
    ...
    REAL(RKIND), PARAMETER :: TINYLocal=1.0E-5_RKIND 
    LOGICAL :: Lunique
    ...
    ALLOCATE( RAllAtomPosition(ind,ITHREE),SAllAtomName(ind),SAllAtomLabel(ind),&
        RAllOccupancy(ind),SAllWyckoffSymbol(ind),RAllIsoDW(ind),IAllAtomicNumber(ind),RAllAnisoDW(ind),STAT=IErr )
...
IF(my_rank.EQ.0) PRINT*, ind,"RSymMat:  ", SPrintString             
    ...
    SWyckoffSymbol(1)= SAllWyckoffSymbol(1)
    ...
    DO ind=2,IMaxPossibleNAtomsUnitCell
      ...
      DO knd=1,jnd-1 ! check against the unique ones found so far
        IF (SUM(ABS(RAllAtomPosition(ind,:)-RAtomPosition(knd,:))).LE.TINYLocal) THEN ! position same
      ...
      IF (Lunique .EQV. .TRUE.) THEN
        ...
        SWyckoffSymbol(jnd) = SAllWyckoffSymbol(ind)
        jnd=jnd+1
      END IF
    END DO
