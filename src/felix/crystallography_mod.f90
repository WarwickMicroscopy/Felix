...
3       CONTINUE
  ...
  SUBROUTINE UniqueAtomPositions(IErr)
    ...
    ALLOCATE( RAllAtomPosition(Imax,ITHREE),SAllAtomName(Imax),SAllAtomLabel(Imax),&
        RAllOccupancy(Imax),RAllIsoDW(Imax),IAllAtomicNumber(Imax),RAllAnisoDW(Imax),STAT=IErr )
    ! Unique atom positions reduced from above
    ALLOCATE( RUniqAtomPosition(Imax,ITHREE),SUniqAtomName(Imax),SUniqAtomLabel(Imax),&
        RUniqOccupancy(Imax),RUniqIsoDW(Imax),IUniqAtomicNumber(Imax),RUniqAnisoDW(Imax),STAT=IErr )
    ...
    SAtomLabel(jnd)= SUniqAtomLabel(1:jnd)
    SAtomName(jnd)= SUniqAtomName(1:jnd)
    RIsoDW(jnd) = RUniqIsoDW(1:jnd)
    ROccupancy(jnd) = RUniqOccupancy(1:jnd)
    IAtomicNumber(jnd) = IUniqAtomicNumber(1:jnd)
    IAnisoDW(jnd) = RUniqAnisoDW(1:jnd)
        
    ! Finished with these variables now
    DEALLOCATE(RAllAtomPosition, SAllAtomName, RAllOccupancy, RAllIsoDW, &
         IAllAtomicNumber, RAllAnisoDW, RUniqAtomPosition, SUniqAtomName, &
         RUniqOccupancy, RUniqIsoDW, IUniqAtomicNumber, RUniqAnisoDW, STAT=IErr)
