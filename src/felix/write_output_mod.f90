...
MODULE write_output_mod
  ...
  SUBROUTINE WriteIterationOutput(IFrame, IErr)
    ...
    USE RPARA, ONLY : RobsHKL,Rhkl, RImageSimi, RInitialThickness, RDeltaThickness,&
    ...
    DO ind = 1,INoOfHKLsFrame!the number of reflections in both felix.hkl and the beam pool
      ...
      ! its index in the felix.hkl list is IhklsAll(ind), its g-vector is RobsHKL(IhklsAll(ind))
