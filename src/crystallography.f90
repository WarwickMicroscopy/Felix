...
!---------------------------------------------------------------
  ...
  LOGICAL :: Lunique
  CHARACTER*2, DIMENSION(:), ALLOCATABLE :: SAllAtomSymbol 
  ...
  ALLOCATE(SAllAtomSymbol(SIZE(RSymVec,1)*SIZE(RBasisAtomPosition,1)),STAT=IErr)  
  ...
  DO ind=1, SIZE(RSymVec,1)  
    DO jnd=1, SIZE(RBasisAtomPosition,1)
      RAllAtomPosition(knd,:)= MATMUL(RSymMat(ind,:,:),RBasisAtomPosition(jnd,:)) &
             + RSymVec(ind,:)
      SAllAtomSymbol(knd) = SBasisAtomSymbol(jnd)
  ...
  SAtomSymbol(1)= SAllAtomSymbol(1)
  ...
  DO ind=2,IMaxPossibleNAtomsUnitCell
    ...
    DO knd=1,jnd-1!check against the unique ones found so far
      ...
      IF ( (SUM(ABS(RAllAtomPosition(ind,:)-RAtomPosition(knd,:))).LE.TINY).AND.&  !position is the same
         (SAllAtomLabel(ind).EQ.SAtomLabel(knd)) ) THEN !label is the same too, so not unique
		Lunique=.FALSE.
		EXIT
		END IF
    END DO
	IF (Lunique .EQV. .TRUE.) THEN
      ...
      SAtomSymbol(jnd)= SAllAtomSymbol(ind)
  ...
  DEALLOCATE(RAllAtomPosition,SAllAtomSymbol,RAllOccupancy,RAllIsoDW,IAllAtomicNumber,RAllAnisoDW)
  IF( IErr.NE.0 ) THEN
